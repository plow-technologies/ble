...
instance S.Serialize a => Encoded (MethodHandlerT IO a)
                                  (MethodHandlerT IO BS.ByteString) where
  encoded o = S.encode <$> o

instance (S.Serialize a, S.Serialize b)
  => Encoded (a -> MethodHandlerT IO b)
             (BS.ByteString -> MethodHandlerT IO BS.ByteString) where
